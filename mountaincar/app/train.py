...
def main():
    ...
    demonstrations = np.load(file=f"{BASE_PATH}/expert_demo/expert_demo.npy")
    ...
    for episode in range(60000):
        ...
        if episode % 1000 == 0:
            ...
            pylab.savefig(f"{BASE_PATH}/learning_curves/app_eps_60000.png")
            np.save(f"{BASE_PATH}/results/app_q_table", arr=q_table)

        if episode % 5000 == 0:
            # optimize weight per 5000 episode
            status = "infeasible"
            temp_learner = calc_feature_expectation(
                feature_num, gamma, q_table, demonstrations, env
            )
