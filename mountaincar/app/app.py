...
class FeatureEstimate:
    ...
    def gaussian_function(self, x, mu):
        return np.exp(-np.power(x - mu, 2.0) / (2 * np.power(1.0, 2.0)))
    ...
    def get_features(self, state):
        ...
        for i in range(int(self.feature_num / 2)):
            # position
            self.feature[i] = self.gaussian_function(
                state[0], env_low[0] + i * env_distance[0]
            )
            # velocity
            self.feature[i + int(self.feature_num / 2)] = self.gaussian_function(
                state[1], env_low[1] + i * env_distance[1]
            )
